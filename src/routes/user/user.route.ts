...
import { User, IParam, IQuery } from './user.type'
import { findAll, find, create, update, remove } from './user.service'

export default (app: FastifyInstance) => {
    app.post<{ Params: IParam; Body: User }>(
        '/api/user/:id',
        async (request, reply) => {
    ...
    app.put<{ Params: IParam; Body: User }>(
        '/api/user/:id',
        async (request, reply) => {
            const id = Number(request.params.id)
            const user = await find(id)
